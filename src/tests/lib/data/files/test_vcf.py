...
class TestVcf(unittest.TestCase):
    def setUp(self):
        ...
        with open(os.path.join(self.tempdir,"in.withoutcontigs.vcf"),'w') as hotspots:
            ...
            hotspots.write("#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\n")
            hotspots.write("chr7\t55249071\t.\tC\tT\t4399\tPASS\tDP4=17465,17450,117,118;AD=34915,235;AF=0.5")
    ...
    def test_create_sample_format_from_info(self):
        ...
        with open(os.path.join(self.tempdir,"sample.vcf"), 'r') as output_vcf:
             ...
             self.assertTrue(lines[3].startswith("##INFO=<ID=AF,"))
             self.assertTrue(lines[4].startswith("##INFO=<ID=AD,"))
             self.assertEqual(lines[5].rstrip(), "##contig=<ID=chr7,length=158821424,assembly=hg19>")
             self.assertTrue(lines[6].startswith("##FORMAT=<ID=AF,"))
             self.assertTrue(lines[7].startswith("##FORMAT=<ID=AD,"))
             self.assertTrue(lines[8].startswith("##FORMAT=<ID=DP,"))
             self.assertTrue(lines[9].startswith("##FORMAT=<ID=DP4,"))
             self.assertTrue(lines[10].startswith("##FORMAT=<ID=GT,"))
             self.assertEqual(lines[11], "#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\tsample1\n")
             self.assertEqual(lines[12].rstrip(), "chr7\t55249071\t.\tC\tT\t4399\tPASS\tDP4=17465,17450,117,118;AD=34915,235;AF=0.5\tGT:AF:DP4:DP:AD\t./1:0.5:17465,17450,117,118:35150:34915,235")
    ...
    def test_add_contigs_to_header(self):
        ...
        with open(os.path.join(self.tempdir,"out.withcontigs.vcf"), 'r') as output_vcf:
            ...
            self.assertTrue("INFO=<ID=AF" in lines[3])
            self.assertTrue("INFO=<ID=AD" in lines[4])
            self.assertEqual(lines[5].rstrip(), "##contig=<ID=chr7,length=158821424,assembly=hg19>")
